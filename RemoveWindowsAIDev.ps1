...
if ($nonInteractive) {
    if (!($AllOptions) -and (!$Options -or $Options.Count -eq 0)) {
        throw 'Non-Interactive mode was not supplied with any options... Please use -Options or -AllOptions when using Non-Interactive Mode'
        exit
    }
}
...
#can prob just keep this part here for interactive and non interactive
